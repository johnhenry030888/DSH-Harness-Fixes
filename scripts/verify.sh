...
gate_warn() {
  if [ "$VERIFY_COMPAT" = "1" ] || [ ! -f .scaffold.json ]; then
    warn "$1"
  else
    fail "$1"
  fi
...
if [ ! -f README.md ] && [ ! -f README.template.md ]; then
  fail "README.md missing"
fi
[ -f STATE.md ] || gate_warn "STATE.md missing (long-horizon bookmark)"
[ -f mise.toml ] || gate_warn "mise.toml missing (toolchain not pinned)"
if [ "$has_py" -eq 1 ]; then
  if command -v ruff >/dev/null 2>&1; then
    ruff check . || fail "ruff check failed"
    ruff format --check . || fail "ruff format --check failed"
  else gate_warn "ruff not found — python lint gate skipped"; fi
else say "skip: python lint gate (no .py files)"; fi
if [ "$has_js" -eq 1 ]; then
  if command -v biome >/dev/null 2>&1; then
    biome check . --config-path "${BIOME_CONFIG_PATH:-$PWD/biome.json}" || fail "biome check failed"
  else gate_warn "biome not found — js lint gate skipped"; fi
else say "skip: js lint gate (no JS/TS files)"; fi
if [ -f tsconfig.json ]; then
  if command -v tsc >/dev/null 2>&1; then
    tsc --noEmit || fail "tsc --noEmit failed"
  else gate_warn "tsc not found — type gate skipped"; fi
else say "skip: type gate (no tsconfig.json)"; fi
if [ -f package.json ]; then
  if command -v npm >/dev/null 2>&1; then npm test --if-present --silent || fail "npm test failed"; else gate_warn "npm not found — node gate skipped"; fi
...
if [ -f pyproject.toml ]; then
  ...
  else gate_warn "no python runner — python gate skipped"; fi
else say "skip: python gate (no pyproject.toml)"; fi
if [ -f Cargo.toml ]; then
  if command -v cargo >/dev/null 2>&1; then cargo test --quiet || fail "cargo test failed"; else gate_warn "cargo not found — rust gate skipped"; fi
else say "skip: rust gate (no Cargo.toml)"; fi
if [ -f go.mod ]; then
  if command -v go >/dev/null 2>&1; then go test ./... || fail "go test failed"; else gate_warn "go not found — go gate skipped"; fi
...
else gate_warn "scripts/audit-secrets.sh missing — secret gate skipped"; fi
if [ -d .git ]; then git status --short || fail "git status failed"; else gate_warn "not a git repo"; fi
...
if [ "$is_ui" -eq 0 ]; then
  ...
  [ -f docs/stages.md ] || gate_warn "stage framework missing (docs/stages.md)"
  [ -f docs/domain.md ] || gate_warn "Stage 1 incomplete (docs/domain.md missing)"
  [ -f docs/app-map.md ] || gate_warn "Stage 2 incomplete (docs/app-map.md missing)"
  if [ -f docs/app-map.md ]; then
    ...
    else gate_warn "app-map validator skipped (scripts/validate_app_map.py or python3 missing)"; fi
  fi
  if [ -f scripts/check-tokens.sh ]; then
    sh scripts/check-tokens.sh || fail "check-tokens.sh failed"
  else gate_warn "scripts/check-tokens.sh missing - token gate skipped"; fi
  [ -f docs/components.md ] || gate_warn "Stage 3 incomplete (docs/components.md missing)"
  has_impl=0
  # Canonical UI-source predicate (identical in scripts/check-tokens.sh - keep in sync).
